...
MeanStructure(state::SemImpliedState) = MeanStructure(implied(state))
ApproximateHessian(state::SemImpliedState) = ApproximateHessian(implied(state))
...
mutable struct Sem{O <: SemObserved, I <: SemImplied, L <: SemLoss} <:
    ...
    function Sem(observed::O, implied::I, loss::L) where {O, I, L}
        # check integrity
        observed_vars(observed) == observed_vars(implied.ram_matrices) ||
            throw(ArgumentError("Observed and implied variables do not match."))

        return new{O,I,L}(observed, implied, loss)
    end
end
